...
from .transformer_model import AnomalyDetectionModel
...
def load_test_data(file_path, dataset_name):
    ...
    try:
        ...
        hsi_data = data[data_key] / np.max(data[data_key])
        ...
        # Apply PCA as in preprocess_data
        pca = PCA(n_components=30 if dataset_name != 'indian' else 40)
...
def evaluate_ae_model(model, test_data, test_labels, threshold=0.01):
    ...
    # Assuming test_labels are binary with 1 for anomaly and 0 for normal
    acc = accuracy_score(test_labels, predictions)
    cm = confusion_matrix(test_labels, predictions)
